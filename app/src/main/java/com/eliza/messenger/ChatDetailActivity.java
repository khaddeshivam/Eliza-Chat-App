...
public class ChatDetailActivity extends AppCompatActivity {
    ...
    private DocumentReference typingRef;
    ...
    private void setupRecyclerView() {
        ...
        messagesAdapter.setOnMessageClickListener((message, position) -> {
    ...
    private void setupFirestoreListeners() {
        ...
        typingRef = db.collection("typing").document(chatId);
        typingRef.addSnapshotListener((documentSnapshot, e) -> {
            if (documentSnapshot != null && documentSnapshot.exists()) {
                boolean typing = documentSnapshot.getBoolean("typing");
                String typingUserId = documentSnapshot.getString("userId");
                updateTypingIndicator(typing, typingUserId);
            }
        });
    }
    ...
    private void sendTypingStatus(boolean isTyping) {
        ...
        typingStatus.put("typing", isTyping);
        typingStatus.put("userId", currentUserId);
        typingRef.set(typingStatus);
    }

    private void updateTypingIndicator(boolean isTyping, String typingUserId) {
        if (isTyping && !typingUserId.equals(currentUserId)) {
            String typingText = getString(R.string.typing_indicator, chatName);
    ...
    private void showDeleteConfirmation(Message message, final int position) {
        new AlertDialog.Builder(this)
    ...
    private void deleteMessage(Message message, final int position) {
        if (message == null) {
            Toast.makeText(this, "Message not found", Toast.LENGTH_SHORT).show();
            return;
        }

        // Delete from Firestore messages collection
        db.collection("chats").document(chatId)
            ...
            .addOnSuccessListener(aVoid -> {
                ...
                if (messagesList.isEmpty()) {
                    db.collection("chats").document(chatId)
                        .update("lastMessage", "")
                        .addOnFailureListener(e -> {
                            Log.e(TAG, "Failed to update chat last message: " + e.getMessage());
                        });
                }
    ...
    protected void onDestroy() {
        super.onDestroy();
        // Clear typing status when activity is destroyed
        sendTypingStatus(false);
    }
}
